...
% sem resistências parasitas
% R1 = 0;
% R2 = 0;
% com resistências parasitas
R1 = 50e-3;
R2 = 50e-3;
...
% RMRAC (para cancelar o distúrbio)
sigma_zero=0.1;
Mo=5;
m2(4)=4;
delta0=0.7;
delta1=1;
% Ganhos e parâmetros para convergência
% Gamma = 10;
% kappa = 2000;

% inicialização dos ganhos (theta) do RMRAC
% SE NÃO OTIMIZAR THETAS (RODA)
ini = -1;
% Thetac(1) = ini;
% Thetas(1) = ini;
% Thetac(2) = ini;
...
for k = 4:60000
   ...
   %Distúrbio ( primeira harmônica da rede - fundamental - 60 Hz )
   d(k)=311*sin(2*pi*k*60*Ts);
    ...
    % saída do sistema (ig - corrente injetada na rede)
    if ( t(k)>=2 )
        % Distúbio da rede
        Lg = 1e-3; % indutância da rede elétrica (com variação paramétrica)
        Fd = [ 0; 0; -1/Lg ];
    end
    ...
    % Ação de controle do controlador PI
    upi(k) = upi(k-1)+Kp*( e0(k)-e0(k-1) ) + Ki*Ts*e0(k);
    
    % Ação de controle total 
    u(k) = upi(k) + ud(k);
