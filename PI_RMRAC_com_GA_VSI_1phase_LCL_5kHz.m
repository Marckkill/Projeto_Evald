...
% Guilherme Hollweg / Paulo Evald
% 
% Algoritmo PI-RMRAC aplicado na planta do VSI com LCL monofasico 
% para obtencao dos ganhos otimos com GA
%
% Data: 31/03/2022
%
% Ultima Modificacao: Paulo Evald (06/04/2022)
%========================================================================
function PI_RMRAC_com_GA_VSI_1phase_LCL_5kHz()
    ...
    %% Parte 2 Configuracao do Algoritmo de Otimizacao
    ...
    options = optimoptions('ga',...
                   ...
                   'Display', 'iter',... %); 
                   'PlotFcn', @gaplotbestf); % plot fitness function

    % rng default % For reproducibility
    intcon = 1;   % Integer variables
 
    A = [];     % A and b - matrix for linear inequality constraints
    b = [];     % Set A=[] and b=[] if no linear inequalities exist
    Aeq = [];   % Aeq and beq - matrix for linear equality constraints
    beq = [];   % Set Aeq=[] and beq=[] if no linear equalities exist
    
    % Gamma, kappa, Theta1, Theta2, Theta3, Theta4, Theta5, Thetac, Thetas
    lb = [  1    1   -20   -20   -20   -20  -20  -20  -20 ];           % limite inferior
    ub = [ 1e3  1e3   20    20    20    20   20   20   20 ];           % limite superior
    nonlcon = [];   %nonlinear constraints (if intcon is not empty, use nonlcon = []).
    [x,fval,exitflag,output] = ga(@(x)fobj(x),9,A,b,Aeq,beq,lb,ub,nonlcon,intcon,options);    
